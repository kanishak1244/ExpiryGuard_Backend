...
function createWindow() {
  ...
  mainWindow.webContents.on('did-fail-load', (event, errorCode, errorDescription, validatedURL) => {
    ...
    // Ignore aborted loads (-3) triggered during transition to offline.html
    if (errorCode === -3) return;

    // Show offline screen if navigation failed and not already loading offline screen
  ...
  // Ensure window is shown even if ready-to-show event is delayed or missed
  setTimeout(() => {
    if (mainWindow && !mainWindow.isVisible()) {
      mainWindow.show();
    }
...
function showOfflineScreen() {
  if (!mainWindow || isNavigatingToOffline) return;
  isNavigatingToOffline = true;
  mainWindow.loadFile(path.join(__dirname, 'offline.html')).catch(err => {
    console.error('[DawaiFlow Desktop] Failed to load offline screen:', err);
  }).finally(() => {
    if (mainWindow && !mainWindow.isVisible()) {
      mainWindow.show();
    }
...
function loadAppURL() {
  ...
  mainWindow.loadURL(PRODUCTION_WEB_URL).catch(err => {
    console.warn('[DawaiFlow Desktop] loadURL rejected:', err?.message || err);
  });
}
